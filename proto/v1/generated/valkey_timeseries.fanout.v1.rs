...
/// / that the shard would have. Under a fused reduction the shard sizes that per
/// / group rather than per series — partials are one per `(group, step)` however
/// / many series fed the group — so a group of sparse series may fold into
/// / `partials` and only a group whose spans are smaller still travels in `raw`.
/// / The coordinator accepts any mix; which the shard chose is its own business.
/// / `series` and `partials` never both appear: a response that carries
/// / per-series values for a fused request, or partials for an unfused one, is
/// / from a corrupt peer.
#[derive(Clone, PartialEq, ::prost::Message)]
