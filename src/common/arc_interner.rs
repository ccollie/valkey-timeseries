...
/// # Example
/// ```rust
/// use valkey_tslib::common::arc_interner::ArcIntern;
///
...
impl<T: Eq + Hash + Send + Sync + 'static + ?Sized> ArcIntern<T> {
    ...
    pub fn num_objects_interned() -> usize {
        CONTAINER
            .pin()
            .get(&TypeId::of::<T>())
            .and_then(|m| m.downcast_ref::<Container<T>>())
...
impl<T: Eq + Hash + Send + Sync + 'static + GetSize> ArcIntern<T> {
    /// The total amount of memory consumed by objects of type T. This may be helpful
    /// in analyzing memory use.
    pub fn total_size() -> usize {
        if let Some(m) = CONTAINER.pin().get(&TypeId::of::<T>()) {
            let m = m.downcast_ref::<Container<T>>().unwrap();
            let mut size = 0;
            for (k, _) in m.pin().iter() {
                size += k.get_size();
            }
            return size;
        }
