use super::utils::is_client_read_only;
use rand::prelude::*;
use rand::rng;
...
use valkey_module::{
    Context, REDISMODULE_NODE_MASTER, VALKEYMODULE_NODE_FAIL, VALKEYMODULE_NODE_ID_LEN,
    VALKEYMODULE_NODE_MYSELF, VALKEYMODULE_NODE_PFAIL, VALKEYMODULE_OK,
    ValkeyModule_FreeClusterNodesList, ValkeyModule_GetClusterNodeInfo,
    ValkeyModule_GetClusterNodesList, ValkeyModuleCtx,
};
...
pub fn get_grouped_cluster_nodes(ctx: &Context) -> ClusterNodeMap {
    ...
    // todo: better capacity estimation
    let mut grouped_nodes: HashMap<String, Vec<ClusterNodeInfo>> =
        HashMap::with_capacity(nodes.len() / 4);
