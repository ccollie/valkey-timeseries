...
impl Clone for SnowflakeIdGenerator {
    fn clone(&self) -> Self {
        let last_time_millis = self.last_time_millis.load(Ordering::Relaxed);
        SnowflakeIdGenerator {
            epoch: self.epoch,
            last_time_millis: AtomicU64::new(last_time_millis),
            machine_id: self.machine_id,
            node_id: self.node_id,
            idx: AtomicU16::new(self.idx.load(Ordering::Relaxed)),
        }
    }
}
...
impl SnowflakeIdGenerator {
    ...
    fn advance_idx(&self) -> u16 {
        let mut idx = self.idx.fetch_add(1, Ordering::Relaxed);
        if idx >= 4095 {
    ...
    pub fn generate(&self) -> u64 {
        ...
        self.last_time_millis.store(now_millis, Ordering::Relaxed);
