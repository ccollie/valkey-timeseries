...
class TestNotifications(ValkeyTimeSeriesTestCaseBase):
    ...
    def test_compaction_upsert_notifications(self):
        """Test that upserting samples to parent series generates compaction notifications."""

        self.create_subscribe_clients()

        # Create source and destination time series with compaction rule
        source_key = "ts:parent:upsert"
        dest_key = "ts:compaction:upsert:sum"

        self.create_ts_with_compaction_rule(source_key, dest_key, "sum", 30000)

        # Add initial samples
        base_timestamp = int(time.time() * 1000)
        for i in range(2):
            timestamp = base_timestamp + (i * 5000)
            self.client.execute_command("TS.ADD", source_key, timestamp, 10.0)

        # Move to the next bucket to finalize the current one
        next_bucket_timestamp = base_timestamp + 35000
        self.client.execute_command("TS.ADD", source_key, next_bucket_timestamp, 15.0)

        # Clear notifications
        self.collect_notifications(timeout=0.5)

        # Now upsert a sample in the already finalized bucket
        upsert_timestamp = base_timestamp + 2000  # Within first bucket
        self.client.execute_command("TS.ADD", source_key, upsert_timestamp, 50.0)

        # Collect notifications
        notifications = self.collect_notifications(timeout=1.0)

        # Verify we received compaction notifications for recalculation
        compaction_notifications = [
            n for n in notifications
            if n['data'] == 'ts.add:dest' and n['channel'] == dest_key
        ]

        assert len(compaction_notifications) >= 1, f"Expected compaction notification for upsert on '{dest_key}', got: {notifications}"

    def test_compaction_range_removal_notifications(self):
        """Test that removing sample ranges from parent series generates compaction notifications."""

        self.create_subscribe_clients()

        # Create source and destination time series with compaction rule
        source_key = "ts:parent:removal"
        dest_key = "ts:compaction:removal:max"

        self.create_ts_with_compaction_rule(source_key, dest_key, "max", 45000)
        ...
        for i in range(6):
            ...
            self.client.execute_command("TS.ADD", source_key, timestamp, 100.0 + i)

        # Move to next bucket to finalize current buckets
        final_timestamp = base_timestamp + 100000
        self.client.execute_command("TS.ADD", source_key, final_timestamp, 200.0)

        # Clear notifications
        self.collect_notifications(timeout=0.5)

        # Remove a range of samples that affects multiple buckets
        start_remove = timestamps[1]
        end_remove = timestamps[4]

        result = self.client.execute_command("TS.DEL", source_key, start_remove, end_remove)
        assert result >= 1  # Should have removed at least one sample

        # Collect notifications
        notifications = self.collect_notifications(timeout=1.0)

        # Verify we received compaction notifications for affected buckets
        compaction_notifications = [
            n for n in notifications
            if n['data'] == 'ts.add:dest' and n['channel'] == dest_key
        ]

        # Should receive notifications for recalculated buckets
        assert len(compaction_notifications) >= 1, f"Expected compaction notifications for range removal on '{dest_key}', got: {notifications}"
