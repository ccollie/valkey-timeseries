...
class TestTsLabelValues(ValkeyTimeSeriesTestCaseBase):
    ...
    def test_basic_label_values(self):
        """Test basic retrieval of label values"""
        self.setup_test_data(self.client)

        # Get all values for the 'name' label
        result = sorted(self.client.execute_command('TS.LABELVALUES', 'name'))
        assert result == [b'cpu', b'disk', b'memory', b'network']

        # Get all values for the 'type' label
        result = sorted(self.client.execute_command('TS.LABELVALUES', 'type'))
        assert result == [b'temperature', b'throughput', b'usage']
    ...
    def test_label_values_with_time_range(self):
        ...
        self.client.execute_command('TS.CREATE', 'ts_old', 'LABELS', 'name', 'archive', 'age', 'old')
        self.client.execute_command('TS.ADD', 'ts_old', 500, 100)

        self.client.execute_command('TS.CREATE', 'ts_new', 'LABELS', 'name', 'recent', 'age', 'new')
        ...
        result = sorted(self.client.execute_command('TS.LABELVALUES', 'age', 'START', 1000))
        assert result == [b'new']

        # Get values with end time range
        result = sorted(self.client.execute_command('TS.LABELVALUES', 'age', 'END', 1500))
        assert result == [b'old']

        # Get values with both start and end time range
        result = sorted(self.client.execute_command('TS.LABELVALUES', 'name', 'START', 900, 'END', 2000))
        assert result == [b'cpu', b'disk', b'memory', b'network']
    ...
    def test_label_values_with_limit(self):
        ...
        result = self.client.execute_command('TS.LABELVALUES', 'name', 'LIMIT', 2)
        assert len(result) == 2
        assert all(val in [b'cpu', b'disk', b'memory', b'network'] for val in result)

        # Get values for the 'node' label with limit
        result = self.client.execute_command('TS.LABELVALUES', 'node', 'LIMIT', 1)
        assert len(result) == 1
        assert result[0] in [b'server1', b'server2', b'server3']
    ...
    def test_label_values_empty_result(self):
        ...
        result = self.client.execute_command('TS.LABELVALUES', 'name', 'START', 5000)
        assert result == []
    ...
    def test_label_values_with_nonexistent_label(self):
        ...
        result = self.client.execute_command('TS.LABELVALUES', 'nonexistent_label')
        assert result == []
    ...
    def test_label_values_after_label_update(self):
        ...
        self.client.execute_command('TS.CREATE', 'ts_new', 'LABELS', 'name', 'updated', 'type', 'special')
    ...
    def test_label_values_with_empty_database(self):
        ...
        result = self.client.execute_command('TS.LABELVALUES', 'name')
        assert result == []

        result = self.client.execute_command('TS.LABELVALUES', 'type')
        assert result == []