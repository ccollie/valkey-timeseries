...
class TestTimeSeriesMetricNamesCME(ValkeyTimeSeriesClusterTestCase):
    ...
    def setup_test_data(client: ValkeyCluster):
        client.execute_command("TS.CREATE", TS1, "METRIC", "cpu_usage_total", "LABELS", "env", "prod")
        client.execute_command("TS.CREATE", TS2, "METRIC", "mem_usage_bytes", "LABELS", "env", "prod")
        client.execute_command("TS.CREATE", TS3, "METRIC", "cpu_idle_total", "LABELS", "env", "dev")
    ...
    def test_metricnames_cluster_fanout(self):
        ...
        assert result == [b"cpu_idle_total", b"cpu_usage_total", b"mem_usage_bytes"]
