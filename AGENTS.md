...
SERVER_VERSION=unstable ./build.sh --parallel                   # parallel integration tests (serial default)
...
- `src/commands/` — one `ts_<name>.rs` file per command, exporting `ts_<name>_cmd`. Most are
  registered via `#[valkey_module_macros::command({...})]` on the handler (full command info: arity,
  key specs, etc.), picked up by `register_commands`. Only `ts._debug` and `ts._restore` (both
  internal/admin, undocumented) stay in the positional `commands:` table in `src/lib.rs`, because ACL
  categories for the attribute path are re-applied separately by `assign_command_acl_categories`
  (driven by the `COMMAND_ACL_CATEGORIES` table — keep it in sync when adding a command).
- `src/series/` — storage, chunk encodings, compaction, background tasks, indexes, serialization.
  - `chunks/`: three encodings — **Chimp** (default), **Gorilla**, **Uncompressed**
    ...
    encoding is a separate, internal policy (see Warnings below).
