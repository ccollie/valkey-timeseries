...
- Parallel sections inside command handlers run on `common::threads::request_pool()` via
  `.on_request_pool().num_threads(request_par_threads(items, work))` (`RequestPoolPar`; see
  `series/mrange.rs`, `sample_merge.rs`). `on_request_pool()` is the only place that names the runner.
