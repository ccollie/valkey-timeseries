...
- PromQL conformance tests (unit tests, no Valkey server needed — uses in-memory `MemorySeriesQuerier`):
    - Fixtures are `.test` files in `src/promql/promqltest/testdata/` (DSL modeled on Prometheus' `promqltest` format
      with `load`, `eval`, `clear`, `ignore`, `resume` commands).
    - `build.rs` auto-generates one `#[test] fn should_pass_<stem>()` per `.test` file into
      `OUT_DIR/promql_tests_generated.rs`, included by `src/promql/promqltest/mod.rs`. Rerun a build after editing
      fixtures.
    - Run all: `cargo test --features enable-system-alloc -- promql_tests`
    - Run one file: `cargo test --features enable-system-alloc -- should_pass_functions`
    - Run via dev script: `SERVER_VERSION=unstable ./build.sh` (the `cargo test --lib` step covers them).
    - Architecture: `runner.rs` orchestrates; `dsl.rs` parses test commands; `loader.rs` ingests series data;
      `evaluator.rs` executes PromQL via the real `PromqlQuerier`; `assert.rs` compares results.
    - No server or Docker is required — these are pure Rust unit tests.
...
- `src/analysis/` — outlier detection algorithms (`src/analysis/outliers/`) and anomaly-detection/statistical helpers.
...
- Add new commands: create `src/commands/ts_<name>.rs`, add function `ts_<name>_cmd`, then register in `valkey_module!`
  ...
  TS.QUERY, TS.QUERYRANGE, TS.QUERYINDEX, TS.QUERYLABELS, TS.CARD, TS.LABELNAMES, TS.LABELVALUES, TS.METRICNAMES, TS.LABELSTATS, TS.CREATERULE,
...
- Documentation: When adding or modifying commands, remember to update the human-facing docs in `docs/COMMANDS.md`,
  `docs/commands/`, `docs/overview.md`, and the supported list in `README.md`. TS._DEBUG is intentionally undocumented.
